...
namespace Ingpal.BusinessStore.Presentation.Printer
{
    /// <summary>
    /// 打开钱箱类
    /// </summary>
    public class CashHelper
    {
        ...
        /// 没钱箱或打印机原功能都可以正常使用
        /// </summary>
        public void ExecuteOpenCashBoxOperate()
        {
            string l_strDefaultPortName = "";
            string l_strDefaultBrand = "";
            if (LocalSet.ReadLocalSet("PrintSet", "UseCashBox", "0") == "0")
            {//如果不使用钱箱直接退出，默认不使用钱箱
                return;
            }
            ...
            try
            {
                ...
                string l_strNetIP = LocalSet.ReadLocalSet("PrintSet", "NetIP", "localhost");
                string l_strNetPort = LocalSet.ReadLocalSet("PrintSet", "NetPort", "9100");
                string l_strBaudRate = LocalSet.ReadLocalSet("PrintSet", "BaudRate", "38400");
                string l_strDataBits = LocalSet.ReadLocalSet("PrintSet", "DataBits", "128");
                int l_intPortCOM = Int32.Parse(LocalSet.ReadLocalSet("PrintSet", "PortCOM", "2"));
                int l_intBaudRateCOM = Int32.Parse(LocalSet.ReadLocalSet("PrintSet", "BaudRateCOM", "9600"));
                int l_intDataBitsCOM = Int32.Parse(LocalSet.ReadLocalSet("PrintSet", "DataBitsCOM", "8"));
                if (l_strDefaultBrand.IndexOf("BTP") > -1 && l_strDefaultPortName.IndexOf("USB") > -1)
                {
                    OpenBYCashBox();
                }
                else if (l_strDefaultBrand.IndexOf("GP") > -1 && l_strDefaultPortName.IndexOf("IP") > -1)
                {
                    OpenJBCashBox(l_strNetIP, l_strNetPort);
                }
                else if (l_strDefaultBrand.IndexOf("EPSON") > -1)
                {
        ...
        private void OpenJBCashBox(string a_strNetIP, string a_strPort)
        {
            ...
            try
            {
                ...
                byte[] byteA = new byte[] { 0x1B, 0x70, 0x00, 0x80, 0xFF };//据说是佳博所有版本通用命令
                                                                           //byte[] byteA = new byte[] { 0x10, 0x14, 0x01, 0x01, 0x03 };
        ...
        private void OpenEPSONCashBox(int a_strPortName, int a_intBaudRate, int a_intDataBits)
        {
